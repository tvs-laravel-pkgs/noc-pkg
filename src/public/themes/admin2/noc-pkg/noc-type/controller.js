    ...
    controller: function($http, $location, HelperService, $scope, $routeParams, $rootScope, $location) {
        ...
        var dataTable = $('#nocs_list').DataTable({
            ...
            stateLoadCallback: function(settings) {
                ...
                if (state_save_val) {
                    $('#search_noc').val(state_save_val.search.search);
                }
            ...
            scrollCollapse: true,
            ajax: {
                url: url(laravel_routes['getNocList']),
                type: "GET",
                dataType: "json",
                data: function(d) {
                    d.noc_code = $('#noc_code').val();
                    d.noc_name = $('#noc_name').val();
            ...
            columns: [
                ...
                { data: 'code', name: 'nocs.code' },
                { data: 'name', name: 'nocs.name' },
                { data: 'mobile_no', name: 'nocs.mobile_no' },
                { data: 'email', name: 'nocs.email' },
            ],
        ...
        var dataTables = $('#nocs_list').dataTable();
        $("#search_noc").keyup(function() {
        ...
        $scope.deleteConfirm = function() {
            $id = $('#noc_id').val();
            $http.get(
                noc_delete_data_url + '/' + $id,
            ).then(function(response) {
                if (response.data.success) {
                    $noty = new Noty({
                        type: 'success',
                        layout: 'topRight',
                        text: 'NOC Deleted Successfully',
                    }).show();
                    setTimeout(function() {
                        $noty.close();
                    }, 3000);
                    $('#nocs_list').DataTable().ajax.reload(function(json) {});
...
app.component('nocForm', {
    ...
    controller: function($http, $location, HelperService, $scope, $routeParams, $rootScope) {
        get_form_data_url = typeof($routeParams.id) == 'undefined' ? noc_get_form_data_url : noc_get_form_data_url + '/' + $routeParams.id;
        var self = this;
        self.hasPermission = HelperService.hasPermission;
        ...
        $scope.onSelectedCountry = function(id) {
            noc_get_state_by_country = vendor_get_state_by_country;
            $http.post(
                noc_get_state_by_country, { 'country_id': id }
            ).then(function(response) {
                // console.log(response);
        ...
        $scope.onSelectedState = function(id) {
            noc_get_city_by_state = vendor_get_city_by_state
            $http.post(
                noc_get_city_by_state, { 'state_id': id }
            ).then(function(response) {
                // console.log(response);
